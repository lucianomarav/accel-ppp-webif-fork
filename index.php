...
<ul>
    ...
    <li><a href="#tabusers">Todas Conexões</a></li>
    <li><a href="#tabusers-pppoe">Conexões PPPOE</a></li>
    <li><a href="#tabusers-ipoe">Conexões IPOE</a></li>
  ...
  <div id="tabusers-pppoe">
  </div>
  <div id="tabusers-ipoe">
  </div>
  <div id="tablogs">
     ...
     <IFRAME src="http://ip_do_seu_servidor_tailon:8080/" width="100%" height="350" scrolling="no" frameborder="0" align="center"></IFRAME>
  </div>
  <div id="tablogout">
  </div>
</div>
...
function reply_click(obj) {
    ...
    var rate = $(obj).closest("tr").children("td:nth-of-type(10)").text().split("/")[0];
...
function activator (event, ui) {
    ...
    if (tabname == "tabusers-pppoe") {
        $("#loadingdialog").dialog('open');
        $(".ui-dialog-titlebar").hide();
        $.post("data.php", { action: "users-pppoe" }, function (ret) {
            //console.log(ret.output);
            $("#tabusers-pppoe").html('<table id="tusers-pppoe" class="cell-border compact"></table>');
            $("#tabusers-pppoe").prepend('<button id="refreshusers-pppoe">Refresh</button>');
            $("#refreshusers-pppoe").button().click( function (e) {
                $("#loadingdialog").dialog('open');
                $(".ui-dialog-titlebar").hide();
                $("#mainscreen").tabs("option", "active", 0);
                setTimeout(function () { $("#mainscreen").tabs("option", "active", 2); }, 200);
            });

            table = $("#tusers-pppoe").DataTable({
                data: ret.output,
                iDisplayLength: 25,
                bLengthChange: false,
                columns: [
	...
	if (tabname == "tabusers-ipoe") {
        $("#loadingdialog").dialog('open');
        $(".ui-dialog-titlebar").hide();
        $.post("data.php", { action: "users-ipoe" }, function (ret) {
            //console.log(ret.output);
            $("#tabusers-ipoe").html('<table id="tusers-ipoe" class="cell-border compact"></table>');
            $("#tabusers-ipoe").prepend('<button id="refreshusers-ipoe">Refresh</button>');
            $("#refreshusers-ipoe").button().click( function (e) {
                $("#loadingdialog").dialog('open');
                $(".ui-dialog-titlebar").hide();
                $("#mainscreen").tabs("option", "active", 0);
                setTimeout(function () { $("#mainscreen").tabs("option", "active", 3); }, 200);
            });

            table = $("#tusers-ipoe").DataTable({
                data: ret.output,
                iDisplayLength: 25,
                bLengthChange: false,
                columns: [
