...
$secret='$6$rounds=10000$6tykoUgahp$KQ8r8NJPjRu2J2RSvOclv9KmlHv4.bYqqJbKvzWhbgy3Ffr8Ujts1YucIQ5DgC/jBARYq3JCHcGCtuv0CPp3V/';
...
switch($_POST{'action'}) {
    ...
    case 'users-pppoe':
        checkauth();
        $arr = runcmd('accel-cmd show sessions match ifname ppp');
        $strings = explode("\r\n", $arr{'output'});
        $strings1 = [];
        $count = count($strings);
        for ($i = 2; $i < $count-1; $i++) {
	...
	case 'users-ipoe':
        checkauth();
        $arr = runcmd('accel-cmd show sessions match ifname IPoE-*');
        $strings = explode("\r\n", $arr{'output'});
        $strings1 = [];
        $count = count($strings);
        for ($i = 2; $i < $count-1; $i++) {
